...
import Sid from "./Contibuters/AdityaFrontEnd";
import Rahul from "./Contibuters/AdityaBackEnd";
import Footer from "./Footer";
// import database from "./fbase.js";

var request = require("request");
const drawerWidth = 240;
const useStylesMine = makeStyles((theme) => ({
  button: {
...
function ResponsiveDrawer(props) {
  ...
  React.useEffect(() => {
    if (window.location.href.includes("?shared=")) {
      var tstamp = window.location.href.substring(
        window.location.href.indexOf("?shared=") + "?shared=".length
      );
      //console.log(tstamp);
  ...
  function CustomizedProgressBars() {
    ...
    return (
      ...
      >
        <ColorCircularProgress size={120} thickness={0.9} /><br/><br/>
        <span>Please Wait...It will takes some seconds to compile or run code</span>
      </div>
    ...
    function FunButtons() {
      ...
      function handleCompile() {
        ...
        if (Date.now() < 5000 + lastRun) {
          alert(
            "5 seconds pause required between two consecutive compilations"
          );
          return;
        }
        setLastRun(Date.now());
        ...
        request(options, (error, response, body) => {
          ...
          var subMRUrl = "https://ide.geeksforgeeks.org/submissionResult.php";
          var respp = JSON.parse(body);
          var success = respp.status;
          if (success != "SUCCESS") {
            alert("Server error in 313");
            setLoading(false);
            return;
          }
          var sid = respp.sid;
          var optionss = {
            method: "POST",
            url: "https://cors-grayhat.herokuapp.com/" + subMRUrl,
            headers: {
              Origin: "https://ide.geeksforgeeks.org",
            },
            form: {
          ...
          var tout = setInterval(()=>{
            request(optionss,(err,res,body)=>{
              if(err){
                alert('Server error in 334');
                setLoading(false);
                return;
              }
              var resp = {};
              ...
              if (resp.status === 'SUCCESS') clearInterval(tout);
              else return;
              var meta = {
              ...
              var outPut = "";
              if (meta.cmpError != undefined) {
                outPut = "Compile Time Error : " + meta.cmpError + "\n\n";
              }
              if (meta.rntError != undefined) {
      ...
      return (
        <div>
          ...
          <FormControlLabel
            ...
            label="Input Field (On/Off)"
          />
          <p></p>
          <a target="_blank" rel="nofollow" href={shared}>
            {shared}
          </a>
        </div>
      );
      ...
      <div>
        <Paper className={classes.root} style={{ backgroundColor: "WHITE" }}>
          <Typography variant="h5" component="h3" style={{ color: "BLACK" }}>
            Write <span style={{textDecoration: "underline", fontWeight:"bold"}}>{language.toUpperCase()}</span> code here
            <br />
            <FunButtons />
            <Code />
            <br />
  ...
  function Output() {
    ...
    return (
      <div>
        <Paper className={classes.root} style={{ backgroundColor: "WHITE" }}>
          <Typography variant="h5" component="h3" style={{ color: "BLACK", fontWeight:"bold" }}>
  ...
  const classesMine = useStylesMine();
  const drawer = (
    <div style={{ backgroundColor: "#424242" }}>
      <p style={{color: "#fff", fontSize: "18px"}}>Languages Selection List</p>
      <div className={classes.toolbar} />
      <Divider />
      <List>
        {langList.map((text, index) => (
          <ListItem
            onClick={() => handleLanguageToogle(text, index)}
            button
            key={text}
            style={{ color: "White" }}
          >
            <ListItemIcon>
              {
                <img
                  src={require(`./newIcons/${text.split(" ")[0]}.png`)}
                  width="25"
                  height="25"
                  alt={text}
                />
          ...
          <ListItem button key={""}>
            {
              <a
                href={
                  index == 0
                    ? "https://github.com/prakashaditya13"
                    : "https://github.com/prakashaditya13"
                }
              >
                {text}
  ...
  const [homeColor, changeColor] = React.useState("primary");
        ...
        <div className={classes.root}>
          ...
          <HideOnScroll threshold={400}>
            <AppBar
              ...
              style={{ background: "#536dfe", boxShadow: "none" }}
            >
              <Toolbar>
                <IconButton
                  color="inherit"
                ...
                <Home
                  className={classesMine.button}
                  onClick={(event) => {
                    window.open("https://github.com/prakashaditya13/", "_self");
            ...
            <Typography variant="h5" align="center">
              <div>
                <h2 style={{ color: "White", textDecoration: "underline" }}>
                  Design and Development of an "Online Compiler"
                </h2>
              </div>
            </Typography>
            <span style={{color: "grey"}}>Developed By <span style={{color: "#fff"}}>Aditya Prakash (0187CS171012)</span> & <span style={{color: "#fff"}}>Aditya Agarwal (0187CS171010)</span></span>
            <br />
            <br />
            <div>
              <Input />
